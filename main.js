...
function canvasDown(e) {
    ...
    // 记录开始的坐标
    point.s.x = clientX - e.target.offsetLeft
    point.s.y = clientY - e.target.offsetTop
    // 若不是 pen 状态就显示辅助线
    if (type !== 'pen') {
...
function canvasMove(e) {
    ...
    clientX -= offsetCanvas.left
    clientY -= offsetCanvas.top
    let canvasX = clientX - t.offsetLeft
    let canvasY = clientY - t.offsetTop
...
function canvasUp(e) {
    ...
    clientY -= offsetCanvas.top
    // 获取结束的坐标
    point.e.x = clientX - e.target.offsetLeft
    point.e.y = clientY - e.target.offsetTop
    // 由于用户有可能从右下向左上绘制，需要将其转化为正数，否则辅助线会显示错误
